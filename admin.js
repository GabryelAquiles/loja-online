...
import { 
    ...
    deleteDoc, doc, query, where, getDocs, updateDoc 
...
function renderizarLinksAdmin() {
    ...
    div.innerHTML = linksTemporarios
        .map(
            (l, i) => `
        <div class="item-row">
...
document
    ...
    .addEventListener("click", async () => {
        ...
        if (!slug) {
            alert("ID da Loja (Slug) é obrigatório!");
            return;
        }
        ...
        try {
            ...
            if (!snap.empty) {
                await updateDoc(
                    doc(db, "config_lojas", snap.docs[0].id),
                    dados
                );
            } else {
        ...
        btn.innerText = "ATUALIZAR IDENTIDADE DA LOJA";
        btn.disabled = false;
    });
...
document
    ...
    .addEventListener("click", async () => {
        ...
        if (!nome || !file || !slug) {
            alert("Preencha Nome, Imagem e o ID da Loja!");
            return;
        }

        btn.innerText = "ENVIANDO CATALOGO...";
        btn.disabled = true;
...
// Carrega dados quando sair do campo "loja_id"
document
    .getElementById("loja_id")
    .addEventListener("blur", carregarTudo);
